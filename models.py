...
class VGG(tf.keras.models.Model):
    ...
    def call(self, inputs):
        ...
        content_outputs, style_output = self.vgg(preprocessed_input)
        return content_outputs, style_output
...
class Encoder(tf.keras.models.Model):
    def __init__(self, content_layer):
        super().__init__()
        vgg = VGG19(include_top=False, weights='imagenet')
        self.vgg = tf.keras.Model([vgg.input], [vgg.get_layer(content_layer).output])
        self.vgg.trainable = False
...
def adaptive_instance_normalization(content_feat, style_feat, epsilon=1e-5):
    ...
    style_std = tf.math.sqrt(style_var + epsilon)

    norm_content_feat = tf.nn.batch_normalization(
        content_feat, mean=content_mean, variance=content_var, offset=style_mean,
        scale=style_std, variance_epsilon=epsilon)
...
class TransferNet(tf.keras.Model):
    ...
    def call(self, content_image, style_image, alpha=1.0):
        t = self.encode(content_image, style_image, alpha) # truth
        g_t = self.decode(t) # ground truth
        return g_t
